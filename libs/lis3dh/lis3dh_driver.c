...
u8_t LIS3DH_ReadReg(u8_t Reg, u8_t *Data)
{
    ...
    uint8_t buffer[2] = {Reg | LIS3DH_READBIT , 0};
    HAL_StatusTypeDef s = HAL_SPI_TransmitReceive(&hspi1, buffer, buffer, 2, 100);
    *Data = buffer[1];
...
{
    HAL_GPIO_WritePin(ACC_nCS0_GPIO_Port, ACC_nCS0_Pin, GPIO_PIN_RESET);
    uint8_t buff_tx[2] = {WriteAddr, Data};

    /* Check if we got an ACK or TIMEOUT error */
    HAL_StatusTypeDef s = HAL_SPI_Transmit(&hspi1, buff_tx, 2, 10);
    HAL_GPIO_WritePin(ACC_nCS0_GPIO_Port, ACC_nCS0_Pin, GPIO_PIN_SET);
    return s == HAL_OK ? 1 : 0;
}
 ...
 * @return u8_t
 */
u8_t LIS3DH_FIFO_ReadWithDMA(u8_t *Data, u16_t Size)
{
    HAL_GPIO_WritePin(ACC_nCS0_GPIO_Port, ACC_nCS0_Pin, GPIO_PIN_RESET);
    uint8_t buffer = LIS3DH_OUT_X_L | LIS3DH_READBIT | LIS3DH_MSBIT;  // LIS3DH_MSBIT

    HAL_StatusTypeDef s = HAL_SPI_Transmit(&hspi1, &buffer, 1, 10);
    if (s != HAL_OK)
    {
        HAL_GPIO_WritePin(ACC_nCS0_GPIO_Port, ACC_nCS0_Pin, GPIO_PIN_SET);
        return 0;
    }
    // s = HAL_SPI_Receive(&hspi1, Data, Size, 10);

    // This function is non-blocking
    s = HAL_SPI_Receive_DMA(&hspi1, Data, Size);
    // CS reset after end of transfer
    // HAL_GPIO_WritePin(ACC_nCS0_GPIO_Port, ACC_nCS0_Pin, GPIO_PIN_SET);
    return s == HAL_OK ? 1 : 0;
}
/* Private functions ---------------------------------------------------------*/
...
/*******************************************************************************
* Function Name  : LIS3DH_GetODR
* Description    : Gets LIS3DH Output Data Rate
* Input          : Output Data Rate
* Output         : None
* Return         : Status [MEMS_ERROR, MEMS_SUCCESS]
*******************************************************************************/
status_t LIS3DH_GetODR(LIS3DH_ODR_t *ov){
  u8_t value;

  if( !LIS3DH_ReadReg(LIS3DH_CTRL_REG1, &value) )
    return MEMS_ERROR;

  *ov = (LIS3DH_ODR_t)((value & 0xF0) >> 4);
...
/*******************************************************************************
* Function Name  : LIS3DH_GetFullScale
* Description    : Gets the LIS3DH FullScale
* Input          : None
* Output         : LIS3DH_FULLSCALE_2/LIS3DH_FULLSCALE_4/LIS3DH_FULLSCALE_8/LIS3DH_FULLSCALE_16
* Return         : Status [MEMS_ERROR, MEMS_SUCCESS]
*******************************************************************************/
status_t LIS3DH_GetFullScale(LIS3DH_Fullscale_t *fs) {
  u8_t value;

  if( !LIS3DH_ReadReg(LIS3DH_CTRL_REG4, &value) )
    return MEMS_ERROR;

  value = (value >> LIS3DH_FS);
  value &= 0x3;
  *fs = value;
