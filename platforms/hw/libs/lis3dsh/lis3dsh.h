...
extern "C"
{
    ...
    {
        uint8_t
            Interrupt_Request; /* Latch/Pulse Interrupt request into CTRL_REG3 register */
        uint8_t Interrupt_Selection_Enable; /* Interrupt1/2 selection enable into
                                               CTRL_REG3 register */
        uint8_t Interrupt_Signal;      /* Interrupt signal active Low/High into CTRL_REG3
                                          register */
        uint8_t State_Machine1_Enable; /* State Machine 1 enable/disable into CTRL_REG1
                                          register */
        uint8_t State_Machine1_Interrupt; /* State Machine 1 routed Interrupt into
                                             CTRL_REG1 register */
        uint8_t State_Machine2_Enable;    /* State Machine 2 enable/disable into CTRL_REG2
                                             register */
        uint8_t State_Machine2_Interrupt; /* State Machine 2 routed Interrupt into
                                             CTRL_REG1 register */
 ...
 *             1 - INT1 (DRDY) signal enabled (signal pin fully functional) DR_EN bit in
 *CTRL_REG3 register should be taken into account too 2 VLIFT: 0 - Vector filter disabled
 *(Default) 1 - Vector filter enabled 1 Reserved 0 STRT: Soft Reset 0 - (Default) 1 - it
 *resets the whole internal logic circuitry. It automatically returns to 0.
 *******************************************************************************/
 ...
 *             0: disable (Default)
 *             1: enable
 *  5 STP_WTM: Stop on Watermark - FIFO depth can be limited at the Watermark value, by
 *setting to �1� the STP_WTM bit. 0: disable (Default) 1: enable 4 IF_ADD_INC: Register
 *address automatically increased during a multiple byte access with a serial interface
 *(I2C or SPI) 0: disable (Default) 1: enable 3 I1_EMPTY: Enable FIFO Empty indication on
 *INT1 pin. 0: disable (Default) 1: enable 2 I1_WTM: FIFO Watermark interrupt on INT1 pin.
 *            0: disable (Default)
 *            1: enable
 ...
 *overwrites the older one (circular buffer). 0    |    1   |    1   | Stream mode until
 *trigger is de-asserted, then FIFO mode. 1    |    0   |    0   | Bypass mode until
 *trigger is de-asserted, then Stream mode. 1    |    0   |    1   | Not to use. 1    | 1
 *|    0   | Not to use. 1    |    1   |    1   | Bypass mode until trigger is
 *de-asserted, then FIFO mode.
 *
 *  4:0 WTMP4-WTMP0: FIFO Watermark pointer. It is the FIFO depth when the Watermark is
 *enabled
 *******************************************************************************/
 ...
 *          1: constant shift enabled (difference between current data and constant
 *values) 2 THR3_MA: 0: no action (Default) 1: threshold 3 enabled for axis and sign mask
 *reset (MASK2_A) 1 R_TAM: Next condition validation flag 0: mask frozen on the axis that
 *triggers the condition (Default) 1: standard mask always evaluated 0 SITR: 0: no actions
 *(Default) 1: STOP and CONT commands generate an interrupt and perform output actions as
 *OUTC command.
 *******************************************************************************/
